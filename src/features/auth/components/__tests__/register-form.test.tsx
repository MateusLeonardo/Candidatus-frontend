...
import * as mutationRegisterUserModule from "../../hooks/use-mutation-register-user";
...
vi.mock("../../hooks/use-mutation-register-user", () => ({
  useMutationRegisterUser: vi.fn(),
}));
...
describe("RegisterForm", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(
      mutationRegisterUserModule.useMutationRegisterUser,
  ...
  it("deve retornar erro caso senha seja inválida", async () => {
    ...
    await waitFor(() => {
      expect(mockMutate).not.toHaveBeenCalled();
      expect(
        screen.getAllByText("A senha deve ter pelo menos 6 caracteres").length,
